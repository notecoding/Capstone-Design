...
| 결과 시각화 | AI 생성 확률 게이지, 의심 프레임 타임라인, 모듈별 레이더 차트 + 상세 표 |
...
│   │   ├── AnalysisChart.jsx    # 모듈별 레이더 차트 + 상세 표
│   │   └── index.js
│   └── history/
...
```json
{
  ...
  "result": {
    ...
    "details": [
      { "module": "clip", "module_name": "프레임 유사도", "score": 0.94, "status": "ok", "reason": "..." },
      { "module": "frequency", "module_name": "주파수 분석", "score": 0.81, "status": "ok", "reason": "..." },
      { "module": "metadata", "module_name": "메타데이터", "score": 0.25, "status": "ok", "reason": "..." }
    ],
    "analysis_details": {
    ...
    "evidence_frames": [
      { "timestamp": 3.2, "image_url": "/storage/results/{task_id}/frame_3_2.jpg", "probability": 0.94, "tags": [] }
    ]
  }
}
```
...
`evidence_frames[].image_url` 기준으로 처리합니다.
절대경로(`/storage/...`)인 경우 `VITE_API_URL`을 앞에 붙입니다.
...
백엔드가 실행하는 모듈은 `clip`, `frequency`, `metadata` 3개입니다.
`confidence`는 세 모듈의 가중 평균입니다 (clip 60% · frequency 25% · metadata 15%).
...
- [ ] 분석 타겟 백엔드 연동 (선택한 모듈만 실행 및 결과 표시)
- [ ] 분석 대기 화면 작업 목록 타겟 연동 (선택 항목만 표시)
- [ ] 홈 통계 수치 실측값으로 업데이트
