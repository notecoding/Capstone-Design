...
export default function VideoUpload({ onSubmit }) {
  ...
  function handleSubmit() {
    ...
    if (!file) {
      setError("파일을 선택하거나 링크를 입력해 주세요");
      return;
    }
  ...
  return (
    <div
      className="mt-fluid-md max-w-content mx-auto"
      style={{
    ...
    >
      ...
      {isLoading && (
        <div
          className="mt-fluid-sm rounded-full overflow-hidden mb-fluid-sm"
      ...
      <div className="flex gap-fluid-xs flex-wrap mt-fluid-sm mb-fluid-sm">
        {["MP4", "AVI", "MOV", "YouTube"].map((l) => (
          <span
            key={l}
            className="text-fluid-xs px-3 py-1 rounded-full"
            style={{
      ...
      {error && (
        <p
          className="text-fluid-sm mt-fluid-sm"
      ...
      <button
        className="btn-brand mt-fluid-md"
        onClick={handleSubmit}
        disabled={isLoading}
      >
        {isLoading ? `업로드 중... ${progress}%` : "분석 시작하기"}
      </button>
    </div>
  );
}
